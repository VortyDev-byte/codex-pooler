...
Then point `~/.hermes/config.yaml` at the same `/v1` base URL, force the Responses transport mode, and include the `image_gen` block for Hermes image generation and edits. Add the `stt` block to enable speech-to-text through Codex Pooler.
