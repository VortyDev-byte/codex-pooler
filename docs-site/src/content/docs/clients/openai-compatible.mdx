...
Standard `gpt-image-2` generation and edit requests use the native Codex image service. Uploaded edit images are forwarded transiently as image data URLs. The provider determines the returned image dimensions; a requested size does not guarantee exact output dimensions.

Requests using `mask` or `input_fidelity`, and older image model identifiers, retain the Responses image-tool translation. These options are not silently dropped or sent to the native image service. Their availability depends on the selected provider host actually producing image output; a completed text response alone is an image-generation failure.
